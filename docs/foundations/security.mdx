...
Server-side code (route guards, REST middleware) uses the request-scoped `model` to read session data, since each request has its own session.
...
export async function isAdmin (model, next, redirect) {
  ...
  const isAdmin = $user.role.get() === 'admin'
