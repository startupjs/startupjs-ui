...
Open `main/pages/PHome/index.jsx` and replace everything inside the `return` block:
...
`Content` with the `padding` prop adds comfortable spacing around your content. `H1` renders a top-level heading. You can explore these components in the docs sidebar under Components.
...
- `$todo.id.get()` reads the document's unique ID (used as the React `key`). Every document has an `id` field that is automatically generated when you call `.add()`.
...
```jsx
import { observer, $, useSub } from 'startupjs'
import { Button, Card, Checkbox, Content, Div, H1, Span } from 'startupjs-ui'
...
```jsx
import { observer, $, useSub } from 'startupjs'
import { Button, Card, Checkbox, Content, Div, H1, Span, TextInput } from 'startupjs-ui'
import { faTimes } from '@fortawesome/free-solid-svg-icons'
