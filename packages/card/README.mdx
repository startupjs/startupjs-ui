import { useState } from 'react'
import { Image } from 'react-native'
import { Sandbox } from '@startupjs-ui/docs'
import Span from '@startupjs-ui/span'
import Collapse from '@startupjs-ui/collapse'
import Button from '@startupjs-ui/button'
import Card, { _PropsJsonSchema as CardPropsJsonSchema } from './index'
import Br from '@startupjs-ui/br'
import Div from '@startupjs-ui/div'
import { faShareAlt } from '@fortawesome/free-solid-svg-icons/faShareAlt'

# Card
Inherits [Div props](/docs/components/Div).

Cards contain content and actions about a single subject. You can pass custom `style` to the root element and provide content as `children`. Since Card inherits from Div, it also supports `onPress` and other Div props.

```jsx
import { Card } from 'startupjs-ui'
```

## Simple example

Although cards can support multiple actions, UI controls, and an overflow menu, use restraint and remember that cards are entry points to more complex and detailed information.

```jsx example
return (
  <Card>
    <Span h4>Card header</Span>
    <Br />
    <Span>Card body</Span>
  </Card>
)
```

## Outlined cards

Set the `variant` prop to `'outlined'` to render a card with a border instead of a shadow.

```jsx example
return (
  <Card variant='outlined'>
    <Span h4>Card header</Span>
    <Br />
    <Span>Card body</Span>
  </Card>
)
```

## Levels of emphasis

The `level` prop controls the shadow intensity (defaults to `1`), providing different levels of visual emphasis. It only applies when the `variant` is `'elevated'` (the default). Accepts values from `0` (no shadow) to `5`.

```jsx example
return (
  <Div>
    <Card level={0}>
      <Span h4>Card Level - 0</Span>
      <Br />
      <Span>Card body</Span>
    </Card>
    <Br />
    <Card>
      <Span h4>Card Level - 1</Span>
      <Br />
      <Span>Card body</Span>
    </Card>
    <Br />
    <Card level={2}>
      <Span h4>Card Level - 2</Span>
      <Br />
      <Span>Card body</Span>
    </Card>
    <Br />
    <Card level={3}>
      <Span h4>Card Level - 3</Span>
      <Br />
      <Span>Card body</Span>
    </Card>
    <Br />
    <Card level={4}>
      <Span h4>Card Level - 4</Span>
      <Br />
      <Span>Card body</Span>
    </Card>
  </Div>
)
```

## Complex interaction (integration with other components)

```jsx example
const style = { maxWidth: 400 }
const imageStyle = { height: 320, resizeMode: 'cover' }
const [collapsed, setCollapsed] = useState(false)
return (
  <Card
    style={style}
  >
    <Image
      source={{ uri: '/img/cat.webp' }}
      style={imageStyle}
    />
    <Br />
    <Span h4>Cat</Span>
    <Br />
    <Span>
      The cat (Felis catus) is a domestic species of small carnivorous mammal.
    </Span>
    <Br />
    <Div row align='between'>
      <Button
        onPress={() => setCollapsed(!collapsed)}
        variant='flat'
      >
        Learn more
      </Button>
      <Button
        pushed
        icon={faShareAlt}
        onPress={() => null}
      >
        Share
      </Button>
    </Div>
    <Br />
    <Collapse open={collapsed} variant='pure' icon={false}>
      <Collapse.Content>
        It is the only domesticated species in the family Felidae and is often referred to as the domestic cat to distinguish it from the wild members of the family. A cat can either be a house cat, a farm cat or a feral cat; the latter ranges freely and avoids human contact. Domestic cats are valued by humans for companionship and their ability to hunt pests such as rodents. About 60 cat breeds are recognized by various cat registries. (source: wiki)
      </Collapse.Content>
    </Collapse>
  </Card>
)
```
<!-- CSSX_STYLE_REFERENCE_START -->
## Styling reference

Use these names from `StartupjsProvider style` or another CSSX provider style layer for app-wide overrides.

### Parts

| Part | Description |
| --- | --- |
| `root` | Root element. |

### CSS variables

| Variable | Default | Description |
| --- | --- | --- |
| `--Card-padding` | `1rem` | Controls the padding. |
| `--Card-radius` | `var(--radius-md)` | Controls the radius. |
| `--Card-bg` | `var(--color-background)` | Controls the background. |
| `--Card-outlined-bg` | `var(--color-background)` | Controls the outlined background. |
| `--Card-border-width` | `1px` | Controls the border width. |
| `--Card-border-color` | `var(--color-border)` | Controls the border color. |
<!-- CSSX_STYLE_REFERENCE_END -->

## Sandbox

<Sandbox
  Component={Card}
  propsJsonSchema={CardPropsJsonSchema}
  props={{
    children: [
      <Span h4 key='header'>Card header</Span>,
      <Br key='br'/>,
      <Span key='body'>Card body</Span>
    ],
    onPress: () => alert('"onPress" event on "Card" component'),
  }}
/>
