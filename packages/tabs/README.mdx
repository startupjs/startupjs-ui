import { useState } from 'react'
import Div from '@startupjs-ui/div'
import Tabs, { _PropsJsonSchema as TabsPropsJsonSchema } from './index'
import { Sandbox } from '@startupjs-ui/docs'

# Tabs

Tabs let you switch between different views. Built on top of [react-native-tab-view](https://reactnavigation.org/docs/tab-view/) with the following API changes:

- The `navigationState` prop is replaced by `routes` and `$value`:
  - `routes` has the same structure as `navigationState.routes` -- an array of `{ key, title }` objects.
  - `$value` is a scoped model that controls which tab is active. Omit it to use uncontrolled mode.
- `initialKey` sets the default active tab for uncontrolled usage.
- `onChange` fires when the active tab changes, receiving the new tab key. It replaces `onIndexChange`.
- TabBar props such as `activeColor`, `inactiveColor`, `scrollEnabled`, `tabStyle`, `indicatorStyle`, `labelStyle`, `contentContainerStyle`, `indicatorContainerStyle`, `bounces`, `pressColor`, `pressOpacity`, `renderIcon`, `renderTabBarItem`, `renderIndicator`, `renderBadge`, `onTabPress`, and `onTabLongPress` can be passed directly to the `Tabs` component.
- Advanced TabView configuration such as `options` and `commonOptions` follows the underlying [react-native-tab-view API](https://reactnavigation.org/docs/tab-view/).
- Per-tab test IDs can be set with route `testID` or upstream `options`.
- `Tabs` does not add a wrapper solely for a root `testID`; locate the tablist/tabs semantically, or use route-level `testID` for individual tabs.

The `renderScene` prop is used to render the content for each tab based on the route key.

You can also pass TabView props like `style`, `sceneContainerStyle`, `pagerStyle`, `swipeEnabled`, `tabBarPosition`, `lazy`, `lazyPreloadDistance`, `renderLazyPlaceholder`, `initialLayout`, `keyboardDismissMode`, `onSwipeStart`, `onSwipeEnd`, `options`, and `commonOptions`.

Note: `tabsStyle` is a deprecated alias for `style`. The `renderLabel` prop is also deprecated -- use `renderTabBarItem` instead.

```jsx
import { Tabs } from 'startupjs-ui'
```

## Tabs.Bar

Use the `Tabs.Bar` component inside the `renderTabBar` function to create a custom tab bar.

## Simple example

Define your routes as an array of `{ key, title }` objects and provide a `renderScene` function that returns the content for each tab.

```jsx example noscroll
const [routes] = useState([
  { key: 'first', title: 'First' },
  { key: 'second', title: 'Second' },
])

const FirstRoute = () => (
  <Div style={{ height: 250, backgroundColor: '#ff4081' }} />
)

const SecondRoute = () => (
  <Div style={{ height: 250, backgroundColor: '#673ab7' }} />
)

function renderScene ({ route }) {
  switch (route.key) {
    case 'first':
      return <FirstRoute />;
    case 'second':
      return <SecondRoute />;
  }
}

return (
  <Tabs
    routes={routes}
    renderScene={renderScene}
  />
)
```
<!-- CSSX_STYLE_REFERENCE_START -->
## Styling reference

Use these names from `StartupjsProvider style` or another CSSX provider style layer for app-wide overrides.

### Parts

| Part | Description |
| --- | --- |
| `root` | Root element. |

### CSS variables

This component does not expose component-specific CSS variables.
<!-- CSSX_STYLE_REFERENCE_END -->

## Sandbox

<Sandbox
  Component={Tabs}
  propsJsonSchema={TabsPropsJsonSchema}
  props={(() => {
    const routes = [
      { key: 'first', title: 'First' },
      { key: 'second', title: 'Second' }
    ]

    const FirstRoute = () => (
      <Div style={{ height: 100, backgroundColor: '#ff4081' }} />
    )

    const SecondRoute = () => (
      <Div style={{ height: 100, backgroundColor: '#673ab7' }} />
    )

    function renderScene ({ route }) {
      switch (route.key) {
        case 'first':
          return <FirstRoute />
        case 'second':
          return <SecondRoute />
        default:
          return null
      }
    }

    return {
      routes,
      renderScene
    }
  })()}
  noScroll
/>
