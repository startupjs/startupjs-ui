import { useState } from 'react'
import { css, pug } from 'startupjs'
import { Sandbox } from '@startupjs-ui/docs'
import { faInfoCircle } from '@fortawesome/free-solid-svg-icons/faInfoCircle'
import Div, { _PropsJsonSchema as DivPropsJsonSchema } from './index.tsx'
import Span from '@startupjs-ui/span'
import Icon from '@startupjs-ui/icon'

# Div

Div is a basic building block for layouts. It is a wrapper around `View` with extra features like press handling (`onPress`, `onLongPress`, `onPressIn`, `onPressOut`), layout helpers (`row`, `wrap`, `reverse`, `align`, `vAlign`, `gap`), shadows (`level`), corner shapes (`shape`), spacing (`pushed`, `bleed`), tooltips (`tooltip`, `tooltipStyle`), and built-in animation support. You can pass custom `style`, provide content as `children`, and use `ref` to access the underlying native view. Accessibility and E2E semantics are exposed with `role`, `aria-*`, and `testID`.

```jsx
import { Div } from 'startupjs-ui'
```

## Simple example
```jsx example
return (
  <Div>
    <Span>Div</Span>
  </Div>
)
```

## Clickable

When an `onPress` or `onLongPress` handler is provided, Div automatically becomes a pressable element with visual feedback. The `feedback` prop (defaults to `true`) controls whether hover and active states are shown. You can override the default feedback styles using `hoverStyle` and `activeStyle`. Set `disabled` to `true` to prevent all interactions.

```jsx example
const divStyle = {
  width: 150,
  height: 100,
  alignItems: 'center',
  justifyContent: 'center',
  backgroundColor: '#00AED6'
}
const [counter, setCounter] = useState(0)
return (
  <Div
    style={divStyle}
    onLongPress={() => setCounter(counter + 10)}
    onPress={() => setCounter(counter + 1)}
  >
    <Span>Clicked {counter} times</Span>
  </Div>
)
```

## Layout

You can use the following props to manage layout:

- `row` -- aligns children from left to right (default: `false`)
- `reverse` -- reverses the children order depending on the direction (default: `false`)
- `wrap` -- controls wrapping of children into multiple rows (default: `false`)
- `align` -- controls horizontal alignment (values: `'left'`, `'center'`, `'right'`; row layouts also support `'around'` and `'between'`)
- `vAlign` -- controls vertical alignment (values: `'top'`, `'center'`, `'bottom'`; row layouts also support `'stretch'`, `'start'`, and `'end'`)
- `gap` -- defines the spacing between items on the CSSX spacing scale; passing `true` is equivalent to `2`

```jsx example
return (
  <Div
    style={{ height: 80, backgroundColor: 'yellow' }}
    row
    align='center'
    vAlign='center'
  >
    <Div style={{ width: 40, height: 40, backgroundColor: 'red' }} />
    <Div style={{ width: 40, height: 40, backgroundColor: 'blue' }} pushed />
  </Div>
)
```

## Full width (or height)

Pass the `full` prop to make Div take all remaining space in the parent container (according to its `flex-direction`). This sets `flex: 1` on the element.

```jsx example
return (
  <Div row vAlign='center'>
    <Div full>
      <Span>Tesla Model S</Span>
    </Div>
    <Span description>1000 HP</Span>
  </Div>
)
```

## Indentation between multiple Div

When displaying multiple Divs in a row, use the `pushed` prop to add spacing from the previous sibling. Accepted values are `'s'`, `'m'`, and `'l'`; passing `true` is equivalent to `'m'`. See the Sandbox section below for all available values.

In most cases, it is better to use the `gap` prop on the parent Div to set uniform spacing between all children at once.

```jsx example
const divStyle = {
  width: 100,
  height: 80,
  textAlign: 'center',
  justifyContent: 'center',
  backgroundColor: '#00AED6'
}
return (
  <Div row>
    <Div style={divStyle}>
      <Span>Div 1</Span>
    </Div>
    <Div pushed='s' style={divStyle}>
      <Span>Div 2</Span>
    </Div>
    <Div pushed style={divStyle}>
      <Span>Div 3</Span>
    </Div>
    <Div pushed='l' style={divStyle}>
      <Span>Div 4</Span>
    </Div>
  </Div>
)
```

## Different clickable states

There are two clickable variants, controlled by the `variant` prop (defaults to `'opacity'`):

- `'opacity'` -- on press, the entire component's opacity is decreased.
- `'highlight'` -- on press, the component's background opacity is decreased.

```jsx example
const divStyle = {
  width: 100,
  height: 80,
  textAlign: 'center',
  justifyContent: 'center',
  backgroundColor: '#00AED6'
}
return (
  <Div row>
    <Div style={divStyle} onPress={() => {}}>
      <Span>Div opacity variant</Span>
    </Div>
    <Div style={divStyle} variant='highlight' pushed onPress={() => {}}>
      <Span>Div highlight variant</Span>
    </Div>
  </Div>
)
```

## Levels of emphasis

The `level` prop adds a shadow to the Div for visual emphasis. Accepted values range from `0` (no shadow) to `5`. See the Sandbox section below for all options.

**Important:** Shadows do not work without a background color on mobile devices.

```jsx example
const divStyle = {
  width: 100,
  height: 80,
  textAlign: 'center',
  justifyContent: 'center',
  backgroundColor: 'white'
}
return (
  <Div row>
    <Div style={divStyle} level={1}>
      <Span>Div level 1</Span>
    </Div>
    <Div style={divStyle} level={4} pushed>
      <Span>Div level 4</Span>
    </Div>
  </Div>
)
```

## Tooltip

Pass the `tooltip` prop to display a simple text tooltip on hover. Use `tooltipStyle` to customize the tooltip's appearance.

```jsx example
return pug`
  Div.root(tooltip='Tooltip content')
    Icon(icon=faInfoCircle size='xl')
`

css`
  .root {
    align-self: flex-start;
  }
`
```

## Animations

Div automatically uses React Native Reanimated when a `transition` or `animation` property is detected in the style. This allows you to create smooth animations without any additional setup.

**Note:** When using modifier classes to add transitions/animations dynamically, set `animation: none` in the base styles. This ensures Div uses the animated component from the start.

```jsx example
const [isActive, setIsActive] = useState(false)
return (
  <Div styleName='root' row align='left' vAlign='center' gap>
    <Div styleName={['box', { isActive }]} />
    <Div styleName='button' onPress={() => setIsActive(!isActive)}>
      <Span style={{ color: 'white' }}>Activate</Span>
    </Div>
  </Div>
)
css`
  .box {
    width: 50px;
    height: 50px;
    background-color: red;
    border-color: red;
    transition: background-color 1s, border-width 1s, border-color 1s;
    animation: none;
  }

  .box.isActive {
    border-width: 10px;
    border-color: lime;
    background-color: green;
    animation: shake 0.5s infinite, pulse 1s infinite, radius 1s forwards, shadow 1s infinite;
  }

  .root {
    height: 60px;
  }

  .button {
    padding: 8px 16px;
    background-color: #2962FF;
    border-radius: 4px;
  }

  @keyframes shake {
    0%, 100% {
      transform: translateX(0) translateY(0);
    }
    25% {
      transform: translateX(-10px) translateY(-10px);
    }
    75% {
      transform: translateX(10px) translateY(10px);
    }
  }

  @keyframes pulse {
    50% {
      width: 60px;
      height: 60px;
    }
  }

  @keyframes radius {
    100% {
      border-radius: 30px;
    }
  }

  @keyframes shadow {
    50% {
      box-shadow: 0px 0px 10px green;
    }
  }
`
```
<!-- CSSX_STYLE_REFERENCE_START -->
## Styling reference

Use these names from `StartupjsProvider style` or another CSSX provider style layer for app-wide overrides.

### Parts

This component does not expose named parts.

### CSS variables

| Variable | Default | Description |
| --- | --- | --- |
| `--Div-hoverBg` | `color-mix(in srgb, var(--palette-main-9) 5%, transparent)` | Controls the hover Bg. |
| `--Div-activeBg` | `color-mix(in srgb, var(--palette-main-9) 20%, transparent)` | Controls the active Bg. |
| `--Div-tooltipBg` | `var(--palette-main-7)` | Controls the tooltip Bg. |
| `--Div-tooltipText` | `var(--palette-main-0)` | Controls the tooltip Text. |
| `--Div-radius` | `var(--radius-md)` | Controls the radius. |
| `--Div-disabled-opacity` | `0.5` | Controls the disabled opacity. |
| `--Div-hover-opacity` | `0.8` | Controls the hover opacity. |
| `--Div-active-opacity` | `0.5` | Controls the active opacity. |
| `--Div-hover-bg` | `color-mix(in srgb, var(--color-foreground) 5%, transparent)` | Controls the hover background. |
| `--Div-active-bg` | `color-mix(in srgb, var(--color-foreground) 20%, transparent)` | Controls the active background. |
| `--Div-pushed-s` | `calc(var(--spacing) * 2)` | Controls the small pushed. |
| `--Div-pushed-m` | `calc(var(--spacing) * 4)` | Controls the medium pushed. |
| `--Div-pushed-l` | `calc(var(--spacing) * 6)` | Controls the large pushed. |
| `--Div-bleed-x` | `calc(var(--spacing) * 4)` | Controls the horizontal bleed. |
| `--Div-tooltip-bg` | `var(--color-popover-foreground)` | Controls the tooltip background. |
| `--Div-tooltip-foreground` | `var(--color-popover)` | Controls the tooltip foreground. |
| `--Div-tooltip-radius` | `var(--radius-md)` | Controls the tooltip radius. |
| `--Div-tooltip-shadow` | `var(--shadow-lg)` | Controls the tooltip shadow. |
| `--Div-shadow-1` | `var(--shadow-xs)` | Controls the shadow 1. |
| `--Div-shadow-2` | `var(--shadow-sm)` | Controls the shadow 2. |
| `--Div-shadow-3` | `var(--shadow-md)` | Controls the shadow 3. |
| `--Div-shadow-4` | `var(--shadow-lg)` | Controls the shadow 4. |
<!-- CSSX_STYLE_REFERENCE_END -->

## Sandbox

<Sandbox
  Component={Div}
  propsJsonSchema={DivPropsJsonSchema}
  props={{
    children: (
      <Div style={{
        height: 15 * 8,
        width: 15 * 8,
        margin: 'auto',
        backgroundColor: '#00AED6'
      }} />
    ),
    style: {
      width: 160,
      height: 160,
      backgroundColor: '#2962FF'
    },
    onPress: () => alert('"onPress" event on "Div" component'),
    onLongPress: () => alert('"onLongPress" event on "Div" component')
  }}
/>
