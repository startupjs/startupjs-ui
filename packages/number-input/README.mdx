import { useState } from 'react'
import NumberInput, { _PropsJsonSchema as NumberInputPropsJsonSchema } from './index'
import Div from '@startupjs-ui/div'
import Br from '@startupjs-ui/br'
import { Sandbox } from '@startupjs-ui/docs'

# NumberInput

A numeric input field that lets users enter or edit a number. It supports increment/decrement buttons, min/max constraints, decimal precision via `step`, and unit labels. The `value` prop holds the current number, and `onChangeNumber` is called whenever it changes. You can also pass a `ref` to access the underlying TextInput, use `placeholder` for hint text, and set `returnKeyType` (defaults to `'done'`) for the keyboard return key.

```jsx
import { NumberInput } from 'startupjs-ui'
```

## Simple example

```jsx example
const [value, setValue] = useState()
return (
  <NumberInput
    value={value}
    onChangeNumber={setValue}
  />
)
```

## Disabled

Set `disabled` to `true` to prevent all interactions with the input.

```jsx example
return (
  <NumberInput
    disabled
    value={10}
  />
)
```

## Readonly

Set `readonly` to `true` to display the value as non-editable text.

```jsx example
return (
  <NumberInput
    readonly
    value={10}
  />
)
```

## Step

The `step` prop controls the increment/decrement amount and the number of allowed decimal places. For example, `step={0.001}` allows up to three decimal places. Defaults to `1`.

```jsx example
const [value, setValue] = useState()
return (
  <Div>
    <NumberInput
      value={value}
      step={0.001}
      onChangeNumber={setValue}
    />
  </Div>
)
```

## Minimum and maximum value

Use the `min` and `max` props to constrain the allowed range.

```jsx example
const [value, setValue] = useState()
return (
  <Div>
    <NumberInput
      value={value}
      min={-30}
      max={20}
      onChangeNumber={setValue}
    />
  </Div>
)
```

## Buttons mode

The `buttonsMode` prop controls the layout of the increment/decrement buttons. Available values are `'vertical'` (default), `'horizontal'`, and `'none'` (hides the buttons entirely).

```jsx example
const [valueVertical, setValueVertical] = useState()
const [valueHorizontal, setValueHorizontal] = useState()
const [valueNone, setValueNone] = useState()
return (
  <Div>
    <NumberInput
      buttonsMode='vertical'
      value={valueVertical}
      onChangeNumber={setValueVertical}
    />
    <Br />
    <NumberInput
      buttonsMode='horizontal'
      value={valueHorizontal}
      onChangeNumber={setValueHorizontal}
    />
    <Br />
    <NumberInput
      buttonsMode='none'
      value={valueNone}
      onChangeNumber={setValueNone}
    />
  </Div>
)
```

## Sizes

The `size` prop controls the input dimensions. Available values are `'s'`, `'m'` (default), and `'l'`.

```jsx example
const [valueL, setValueL] = useState()
const [valueM, setValueM] = useState()
const [valueS, setValueS] = useState()
return (
  <Div>
    <NumberInput
      size='s'
      value={valueS}
      onChangeNumber={setValueS}
    />
    <Br />
    <NumberInput
      size='m'
      value={valueM}
      onChangeNumber={setValueM}
    />
    <Br />
    <NumberInput
      size='l'
      value={valueL}
      onChangeNumber={setValueL}
    />
  </Div>
)
```

## Units

The `units` prop displays a label next to the input (such as a currency symbol). By default, units appear on the left. Use `unitsPosition='right'` to place them on the right side.

```jsx example
const [value, setValue] = useState()
return (
  <NumberInput
    value={value}
    units='$'
    onChangeNumber={setValue}
  />
)
```

## Styling

Use the following props to customize the appearance:

- `style` -- styles for the root wrapper
- `inputStyle` -- styles for the text input element
- `buttonStyle` -- styles for the increment/decrement buttons

```jsx example
const [value, setValue] = useState()

return (
  <NumberInput
    style={{
      width: 250
    }}
    buttonStyle={{
      borderWidth: 0
    }}
    inputStyle={{
      borderTopWidth: 0,
      borderLeftWidth: 0,
      borderRightWidth: 0,
      borderRadius: 0
    }}
    placeholder='Enter number'
    value={value}
    onChangeNumber={setValue}
  />
)
```
<!-- CSSX_STYLE_REFERENCE_START -->
## Styling reference

Use these names from `StartupjsProvider style` or another CSSX provider style layer for app-wide overrides.

### Parts

| Part | Description |
| --- | --- |
| `root` | Root element. |
| `button` | Button control. |
| `units` | Units text. |
| `wrapper` | Wrapper element. |

### CSS variables

| Variable | Default | Description |
| --- | --- | --- |
| `--NumberInput-button-border-color` | `var(--TextInput-border-color)` | Controls the button border color. |
| `--NumberInput-units-font-size-l` | `var(--text-base)` | Controls the large units font size. |
| `--NumberInput-units-line-height-l` | `1.5rem` | Controls the large units line height. |
| `--NumberInput-units-readonly-gap` | `0.25rem` | Controls the units readonly gap. |
| `--NumberInput-units-gap` | `0.5rem` | Controls the units gap. |
| `--NumberInput-button-padding-l` | `3rem` | Controls the large button padding. |
| `--NumberInput-button-padding-m` | `2.5rem` | Controls the medium button padding. |
| `--NumberInput-button-padding-s` | `2rem` | Controls the small button padding. |
<!-- CSSX_STYLE_REFERENCE_END -->

## Sandbox

<Sandbox
  Component={NumberInput}
  propsJsonSchema={NumberInputPropsJsonSchema}
  extraParams={{
    step: {
      step: 0.0001
    },
    value: {
      step: 0.0001
    }
  }}
  block
/>
