import { Sandbox } from '@startupjs-ui/docs'
import {
  Table,
  Thead,
  Tbody,
  Tr,
  Th,
  Td,
  TablePropsJsonSchema,
  TheadPropsJsonSchema,
  TbodyPropsJsonSchema,
  TrPropsJsonSchema,
  ThPropsJsonSchema,
  TdPropsJsonSchema
} from './index'
import Span from '@startupjs-ui/span'

# Table

The Table component arranges data in rows and columns. Use `Thead` for the header section, `Tbody` for the body, `Tr` for rows, `Th` for header cells, and `Td` for data cells.

The `Thead`, `Tbody`, `Tr`, `Th`, and `Td` components extend `Div` props, so they support layout and interaction features like `onPress`, `hoverStyle`, and `activeStyle`. The `Thead` component has a `bordered` prop (defaults to `true`) that adds a bottom border. Both `Th` and `Td` support an `ellipsis` prop that collapses text to a single line with an ellipsis -- tap the cell to toggle the full text.

```jsx
import { Table, Thead, Tbody, Tr, Th, Td } from 'startupjs-ui'
```

## Simple example

```jsx example
return (
  <Table>
    <Thead>
      <Tr>
        <Th>
          <Span bold>First</Span>
        </Th>
        <Th>
          <Span bold>Second</Span>
        </Th>
      </Tr>
    </Thead>
    <Tbody>
      <Tr>
        <Td>
          <Span>First 1</Span>
        </Td>
        <Td>
          <Span>Second 1</Span>
        </Td>
      </Tr>
      <Tr>
        <Td>
          <Span>First 2</Span>
        </Td>
        <Td>
          <Span>Second 3</Span>
        </Td>
      </Tr>
    </Tbody>
  </Table>
)
```

## Styling

Since `Tr`, `Th`, and `Td` extend `Div` props, you can apply custom hover and active styles directly. The example below demonstrates hover effects on header cells and body rows.

```jsx example
const tableStyle = {
  backgroundColor: '#fff'
}

const hoverOptions = {
  style: {
    cursor: 'default'
  },
  hoverStyle: {
    backgroundColor: '#ebf8fd'
  },
  activeStyle: {
    opacity: 1,
    backgroundColor: '#ebf8fd'
  },
  onPress: () => undefined
}

return (
  <Table style={tableStyle}>
    <Thead>
      <Tr>
        <Th {...hoverOptions}>
          <Span bold italic>First</Span>
        </Th>
        <Th {...hoverOptions}>Second</Th>
        <Th {...hoverOptions}>Third</Th>
      </Tr>
    </Thead>
    <Tbody>
      <Tr {...hoverOptions}>
        <Td>
          <Span italic>First 1</Span>
        </Td>
        <Td>Second 1</Td>
        <Td>Third 1</Td>
      </Tr>
      <Tr {...hoverOptions}>
        <Td>
          <Span italic>First 2</Span>
        </Td>
        <Td>Second 2</Td>
        <Td>Third 2</Td>
      </Tr>
    </Tbody>
  </Table>
)
```
<!-- CSSX_STYLE_REFERENCE_START -->
## Styling reference

Use these names from `StartupjsProvider style` or another CSSX provider style layer for app-wide overrides.

### Parts

| Part | Description |
| --- | --- |
| `root` | Root element. |

### CSS variables

| Variable | Default | Description |
| --- | --- | --- |
| `--Table-border-color` | `var(--color-border)` | Controls the border color. |
| `--Table-border-width` | `1px` | Controls the border width. |
| `--Table-cell-padding-y` | `0.5rem` | Controls the vertical cell padding. |
| `--Table-cell-padding-x` | `1rem` | Controls the horizontal cell padding. |
<!-- CSSX_STYLE_REFERENCE_END -->

## Sandbox

### Table

export const TableSandbox = (props) => (
  <Table {...props}>
    <Thead>
      <Tr>
        <Th>First</Th>
        <Th>Second</Th>
      </Tr>
    </Thead>
    <Tbody>
      <Tr>
        <Td>First 1</Td>
        <Td>Second 1</Td>
      </Tr>
    </Tbody>
  </Table>
)

<Sandbox
  Component={TableSandbox}
  propsJsonSchema={TablePropsJsonSchema}
/>

### Thead

export const TheadSandbox = (props) => (
  <Table>
    <Thead {...props}>
      <Tr>
        <Th>Header</Th>
      </Tr>
    </Thead>
  </Table>
)

<Sandbox
  Component={TheadSandbox}
  propsJsonSchema={TheadPropsJsonSchema}
/>

### Tbody

export const TbodySandbox = (props) => (
  <Table>
    <Tbody {...props}>
      <Tr>
        <Td>Cell</Td>
      </Tr>
    </Tbody>
  </Table>
)

<Sandbox
  Component={TbodySandbox}
  propsJsonSchema={TbodyPropsJsonSchema}
/>

### Tr

export const TrSandbox = (props) => (
  <Table>
    <Tbody>
      <Tr {...props}>
        <Td>Cell</Td>
        <Td>Cell</Td>
      </Tr>
    </Tbody>
  </Table>
)

<Sandbox
  Component={TrSandbox}
  propsJsonSchema={TrPropsJsonSchema}
/>

### Th

export const ThSandbox = (props) => (
  <Table>
    <Thead>
      <Tr>
        <Th {...props}>Header</Th>
      </Tr>
    </Thead>
  </Table>
)

<Sandbox
  Component={ThSandbox}
  propsJsonSchema={ThPropsJsonSchema}
/>

### Td

export const TdSandbox = (props) => (
  <Table>
    <Tbody>
      <Tr>
        <Td {...props}>Cell</Td>
      </Tr>
    </Tbody>
  </Table>
)

<Sandbox
  Component={TdSandbox}
  propsJsonSchema={TdPropsJsonSchema}
/>
