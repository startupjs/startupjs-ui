...
Button reads its defaults from CSSX variables, so apps can tune it through `StartupjsProvider style` or another `CssxProvider` layer:

- **--Button-height-{size}** -- button height for `xs`, `s`, `m`, `l`, `xl`, and `xxl`.
- **--Button-font-size-{size}** and **--Button-line-height-{size}** -- label typography.
- **--Button-outlined-border-width** and **--Button-outlined-border-color** -- outlined variant border.
- **--Button-disabled-opacity** -- disabled state opacity.
- **--Button-web-native-button** -- controls whether the web root renders with native button semantics.

Use `Button`, `Button:part(text)`, `Button:part(icon)`, and `Button:part(loader)` selectors for app-wide structural overrides.
