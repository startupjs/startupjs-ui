...
The component also accepts a `style` prop for custom styles, `push` to use router push instead of navigate, and `replace` to replace the current history entry instead of adding a new one.
